...
var jokeIntervalMs = 10000;
var jokes = JSON.parse(fs.readFileSync("./server/knock-knock-jokes.json", "UTF-8"));
...
ioSocket.on("connection", function (socket) {
  ...
  socket.on("msg", function (msgData) {
    ...
    var msgL = msgData.msg.toLowerCase();

    if (jokeState == "knock" && msgL === "who's there?") {
      jokeState = "who-there";
      setTimeout(function () {
        var msg = { msg: curJoke.who, when: Date.now() };
        emitToAll("notice", msg);
        msgsCollection.addItem(msg);
      }, 50);
    }
    else if (jokeState == "who-there" && msgL === (curJoke.who + " who").toLowerCase()) {
      jokeState = "who";
      setTimeout(function () {
        jokeState = null;
        var msg = { msg: curJoke.punchline, when: Date.now() };
        emitToAll("notice", msg);
        msgsCollection.addItem(msg);
        if (curCb) { curCb(); }
      }, 50);
...
var curJoke = null;
var curCb = null;
var jokeState = null; // one of: ["knock", "who-there", "who", null]

function postJoke() {
  jokeCount++;

  setupAwaitTaskTimer(jokeIntervalMs, function (callback) {
    jokeCount++;
    jokeState = "knock";

    curJoke = jokes.jokes[Math.floor(jokes.jokes.length * Math.random())];
    curCb = callback;

    var msg = { msg: "Knock, knock.", when: Date.now() };
    emitToAll("notice", msg);
    msgsCollection.addItem(msg);
  });
}
